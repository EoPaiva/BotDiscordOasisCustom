...
import { RecruitmentAssignmentForm } from "../assignment-form";
import { RecruitmentDecisionForm } from "../decision-form";
import { addRecruitmentAdaptation, addRecruitmentNote, evaluateRecruitmentInterview, reanalyzeRecruitmentApplication, recordRecruitmentAnalysisFeedback, scheduleRecruitmentInterview } from "../actions";
...
export default async function RecruitmentDossierPage({ params }: PageProps<"/recruitment/[id]">) {
  ...
  const open = !["APPROVED", "REJECTED", "WITHDRAWN", "EXPIRED"].includes(status);
  const canAssign = ["SUBMITTED", "UNDER_REVIEW"].includes(status) && !application.assigned_to;
  ...
  return <>
    ...
    <div className="recruitment-dossier-grid">
      ...
      <aside className="recruitment-dossier-actions">
        {open && <section className="command-section"><SectionHeader index="A" title="Responsável" meta="Lock otimista" /><p>{application.assigned_to ? <>Assumida por <code>{String(application.assigned_to)}</code>.</> : status === "DRAFT" ? "Aguardando o candidato concluir e enviar a candidatura." : "Ainda sem recrutador responsável."}</p>{canAssign && <RecruitmentAssignmentForm applicationId={Number(application.id)} expectedVersion={Number(application.version)} />}</section>}
        ...
        {open && ["UNDER_REVIEW","INTERVIEW_COMPLETED","FINAL_REVIEW"].includes(String(application.status)) && <section className="command-section"><SectionHeader index="D" title="Decisão final" meta="Somente comando" /><RecruitmentDecisionForm applicationId={Number(application.id)} expectedVersion={Number(application.version)} /></section>}
