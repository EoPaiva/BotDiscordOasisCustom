...
export default async function RecruitmentAiPage() {
  ...
  return <>
    ...
    <MetricStrip items={[
      ...
      { label: "PROVIDER", value: config.provider_ready ? config.provider : "NÃO CONFIGURADO", tone: config.provider_ready ? "success" : "warning" },
    ...
    <div className="dashboard-grid">
      <section className="command-section"><SectionHeader index="01" title="Configuração operacional" meta={`${config.provider} / ${config.model}`} />
        <form action={updateRecruitmentAiConfiguration} className="ai-config-form">
          ...
          {!config.provider_ready && <p className="candidate-notice">Defina o provider e o segredo no ambiente do backend antes de ativar.</p>}
