...
export default async function OfficerCandidaciesPage({ searchParams }: PageProps<"/officer-candidacies">) {
  ...
  return <>
    ...
    <section className="command-section"><SectionHeader index="01" title="Fila dos responsáveis por upamento" meta="Dados internos e relatório consultivo" />
      <form aria-label="Filtros das candidaturas ao Oficialato" className="recruitment-filters"><select aria-label="Filtrar por status" defaultValue={status} name="status"><option value="">Todos os status</option>{["SUBMITTED", "IN_REVIEW", "INTERVIEW_REQUIRED", "APPROVED_CONDITIONAL", "APPROVED", "REJECTED", "RETURNED"].map((item) => <option key={item}>{item}</option>)}</select><input aria-label="Filtrar por ID do responsável" defaultValue={assignedTo} name="assigned_to" inputMode="numeric" pattern="[0-9]*" placeholder="ID do responsável" /><button className="button button-secondary compact" type="submit">Filtrar</button></form>
