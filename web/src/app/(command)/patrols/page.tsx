...
import { dateTime, duration, isoDateTime, isoDuration } from "@/lib/format";
    ...
    <div className="split-operational">
      <section className="command-section">
        ...
        {data.active.length ? <ul aria-label="Patrulhas em andamento" className="patrol-list">{data.active.map((row) => <li key={String(row.id)}><article className="patrol-record">
          ...
          <div className="patrol-time"><strong><time dateTime={isoDuration(data.generated_at - Number(row.started_at ?? data.generated_at))}>{duration(data.generated_at - Number(row.started_at ?? data.generated_at))}</time></strong><span>desde <time dateTime={isoDateTime(row.started_at)}>{dateTime(Number(row.started_at))}</time></span></div>
