...
export default async function SecurityPage() {
  ...
  return <>
    ...
    <section className="command-section"><SectionHeader index="01" title="Modo de emergência" meta="Alterações administrativas são bloqueadas; leitura e auditoria permanecem" />
      {data.lockdown.active && <div className="candidate-blocked"><strong>SECURITY LOCKDOWN ATIVO</strong><p>{data.lockdown.reason ?? "Contenção administrativa em vigor."} {data.lockdown.changed_at ? <>Desde <time dateTime={isoDateTime(data.lockdown.changed_at)}>{dateTime(data.lockdown.changed_at)}</time>.</> : ""}</p></div>}
    ...
    <section className="command-section"><SectionHeader index="02" title="Eventos recentes" meta={`${data.events.length} registros append-only`} />
      <DataTable caption="Eventos recentes de segurança" rows={data.events} columns={[
        { key: "created_at", label: "DATA", render: (row) => <time dateTime={isoDateTime(row.created_at)}>{dateTime(Number(row.created_at))}</time> },
