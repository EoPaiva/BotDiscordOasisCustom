...
## 0. Checkpoint mais recente — cortes 17 e 18 da Fase 57 publicados

**Branch:** `main`. **TDD:** `4a42580`/`74bf8c7` cobrem o instante “desde” e
`c8ebb65`/`5703f8b` cobrem a entrada FIFO. Todos foram enviados para `origin/main` e `private/main`.

**Entregue:** o instante “desde” de cada operação ativa e a coluna `ENTRADA` da fila FIFO em
`/patrols` agora usam `<time dateTime="…">`, preservando texto, cálculo, ordenação, URL, API, RBAC
e regras de negócio.

**Gates locais:** 2 testes focados da Central de Patrulhas; 93 testes em 21 arquivos; lint; typecheck; build;
`npm audit` sem vulnerabilidades; 8 E2E aprovados e 1 cenário visual Firefox intencionalmente
ignorado.

**Produção técnica:** deploy Vercel `dpl_HqFuYxqJM1LeocWRFDtDfR5rr68s`, `READY` em 2026-08-27,
...
bloqueada, a Fase 57 pode avançar localmente em cortes pequenos e fecháveis; dezoito cortes do Centro
...
O protocolo de continuidade foi instalado na raiz. Depois, a Fase 57 avançou em dezoito cortes do
...
`c8ebb65`/`5703f8b` cobrem a coluna `ENTRADA` da fila FIFO. Nenhum contrato, URL, RBAC, API ou regra
...
Nenhuma alteração está em andamento. Transferências, os dezoito cortes acessíveis da Fase 57 e o
