...
Status: **IMPLEMENTADO LOCALMENTE EM 2026-08-22; PUBLICAÇÃO EXTERNA NÃO EXECUTADA**.

Resultado: seis subfases entregues em `web/` e `command_center/`, com ADRs 004–007, migration v16,
autenticação Discord/Auth.js, RBAC revalidado na API, outbox para ações Discord, páginas reais das
áreas Operação/Efetivo/Administração/Inteligência/Sistema, configuração Vercel/Railway, polling
seguro, testes e revisão visual. Supabase, OAuth de produção, domínio e deploy continuam como
rollout externo deliberadamente não provisionado, conforme a própria regra desta especificação.

Fonte: especificação complementar recebida em 2026-08-22 e preservada integralmente abaixo.

## Regras de entrada na fase

- Esta é uma fase futura e deve começar somente depois das fases funcionais já enfileiradas.
- O estado atual continua sendo Python, discord.py e SQLite, sem frontend, API, Supabase ou Railway.
- Supabase/PostgreSQL, Railway, Discord OAuth e realtime são arquitetura-alvo desta especificação,
  não componentes existentes.
- Antes de escrever a interface, usar integralmente a skill de frontend design disponível no
  ambiente.
- Usar o Lovable na fundação visual, prototipação e refinamento das páginas. Começar em modo de
  planejamento; revisar o código/diff gerado antes de integrar ao repositório principal.
- Não publicar o projeto Lovable, provisionar banco, conectar serviços ou expor o sistema sem uma
  fase arquitetural aprovada e validações de segurança.
- Preservar o core de regras de negócio no backend compartilhado. O navegador nunca será a fonte de
  verdade para RBAC, promoção, disciplina, ponto ou sincronização Discord.
- Atualizar `PROJECT_HANDOFF.md` ao concluir cada subfase.
