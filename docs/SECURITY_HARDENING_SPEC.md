# Security Hardening Completo — CHOQUE BGR

Status: **CONCLUÍDO LOCALMENTE EM 2026-08-22 — GATE PÚBLICO FAIL**.

Resultado: os 220 controles foram classificados individualmente em
`docs/SECURITY_CONTROL_MATRIX.md` (164 implementados, 27 não aplicáveis e 29 pendentes). Migration
v19, autenticação HMAC anti-replay, segurança de sessão, lockdown, backups, auditoria Discord,
CI/SBOM, threat model e runbooks foram entregues. O FAIL é deliberado: token Discord ainda precisa
ser regenerado e a infraestrutura externa não possui prova de TLS/MFA/WAF/monitoramento/pentest.

Fonte: especificação complementar recebida em 2026-08-22 e preservada integralmente abaixo.

## Regras de entrada e execução

- Este programa vem depois do Centro de Comando Web, do Sistema de Alistamento, Recrutamento e
  Integridade e do Robô Analista de Candidaturas na ordem formal da fila.
- Segurança básica não será adiada: least privilege, deny-by-default, proteção de secrets,
  autorização server-side, auditoria e testes de regressão são gates de todas as fases anteriores.
- A etapa final consolida threat model, ASVS, hardening da arquitetura completa, testes de segurança,
  CI/CD, infraestrutura, backups, monitoramento, resposta a incidentes e aceite de produção.
- O estado consolidado possui bot Python/discord.py, SQLite, API FastAPI e frontend Next.js/Auth.js.
  Supabase/PostgreSQL, Railway, domínio público, realtime e WAF continuam dependentes do rollout.
- Cada controle deve ser classificado como IMPLEMENTADO, NÃO APLICÁVEL ou PENDENTE, com evidência e
  justificativa; não declarar segurança absoluta.
- Usar Lovable nas superfícies web aplicáveis — especialmente UX de segurança, painel restrito,
  estados e acessibilidade — sempre em modo de planejamento/revisão e com inspeção do diff.
- Nunca enviar ao Lovable tokens, cookies, chaves, dumps, dados pessoais ou configuração real de
  produção. Lovable não substitui revisão de backend, autorização, RLS, threat modeling ou testes.
- Não publicar, provisionar infraestrutura, executar DAST agressivo ou realizar pentest sem escopo e
  autorização explícitos.
- Atualizar `PROJECT_HANDOFF.md`, `docs/PHASE_QUEUE.md` e o relatório de segurança ao concluir
  cada subfase.
