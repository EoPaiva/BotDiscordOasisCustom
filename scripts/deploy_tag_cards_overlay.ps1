...
param(
    ...
    [string]$StageRoot = (Join-Path $env:TEMP 'choque-tag-cards-staging'),
    [switch]$SkipCommit
)
...
foreach ($relative in $tagOverlay) {
    $source = Join-Path $projectRoot $relative
    if (-not (Test-Path -LiteralPath $source)) {
        throw "Arquivo obrigatório da Central de Tags ausente: $source"
    }
...
$required = @(
    (Join-Path $stage 'choque\database.py'),
    (Join-Path $stage 'choque\tags.py'),
    (Join-Path $stage 'cogs\tag_commands.py'),
...
& python -m compileall -q (Join-Path $stage 'choque') (Join-Path $stage 'cogs')
if ($LASTEXITCODE -ne 0) {
    throw 'Falha de compilação no artefato da Central de Tags.'
}
Push-Location $stage
try {
    & python -c "from choque.tags import TagService; from cogs.tag_commands import TagCommands, TagRequestCardView; print('TAG_CARDS_STAGE_IMPORT_OK')"
    if ($LASTEXITCODE -ne 0) {
        throw 'Falha de importação no artefato da Central de Tags.'
    }
} finally {
    Pop-Location
}
...
$manifest | Set-Content -LiteralPath (Join-Path $stage 'TAG_CARDS_OVERLAY_MANIFEST.txt') -Encoding utf8

if ($SkipCommit) {
    Write-Output "DISCLOUD_TAG_CARDS_STAGE_READY app=$App stage=$stage files=$($tagOverlay.Count)"
    return
}
...
Write-Output "DISCLOUD_TAG_CARDS_OVERLAY_DEPLOYED app=$App stage=$stage files=$($tagOverlay.Count)"
